...
void opera_matrices_bucles(float **matriz_x, float **matriz_y, int size)
{
	int i, j, k, a, b;
	for(k=0; k<ITER; k++){
		for(i=0; i<size; i++){ 
	...
	for(k=0; k<ITER; k++)
		for(i=0; i<size; i++){ 
			for(j=0; j<size; j+=4){ 
				...
				b = i + (tmp);
				matriz_y[j][i] = a * matriz_x[j][i] + b;
				matriz_y[j+1][i] = a * matriz_x[j+1][i] + b;
				matriz_y[j+2][i] = a * matriz_x[j+2][i] + b;
				matriz_y[j+3][i] = a * matriz_x[j+3][i] + b;
			}
		}
