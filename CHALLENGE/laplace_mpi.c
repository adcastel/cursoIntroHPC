...
int main(int argc, char *argv[]) {
    ...
    if (NPES != npes){
        if(my_PE_num==0) {
            printf("This code must be run with %d processes", NPES);
        }
        MPI_Finalize();
        exit(1);
    }
  ...
  //      gettimeofday(&start,NULL);
    ...
    if(my_PE_num==whereisthedata){
        printf("\nPE %d: Temperature[%d,%d] position is %f\n", my_PE_num, check_row, check_col, Temperature[1][9951]);
    }
    MPI_Finalize();
}
